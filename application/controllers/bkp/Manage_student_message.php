...
class Manage_student_message extends CI_Controller {
	...
	function index()
	{
		...
		$data['stu_id'] = $user_details->id;
		$college_cat = $user_details->college_cat;
		$college_id = $user_details->id;
		$data['clg_id'] = $user_details->id;
		
		
		$data['unread'] = $this->db->query("select * from td_message_details where student_id='$student_id' and is_student_read=0")->num_rows();	
		
		$data['rows'] = $this->db->query("select a.*,b.*,a.message_id as main_id from td_messages as a inner join td_message_details as b on b.message_id=a.message_id where a.user_id='$student_id' group by b.message_id order by date desc")->result();
		...
		$data['maincontent']=$this->load->view('admin/maincontents/manage-student-message-list-view',$data,true);
	...
	function view($id)
	{
		...
		$update_array = array('is_student_read'=>1);
		...
		if($this->input->post('mode')=='message')
		{
			...
			if($succ2)
			{
				...
				redirect('admin/manage_student_message/view/'.$id);
			}
			else			
			{
		...
		$data['left_sidebar'] = $this->load->view('admin/elements/left-sidebar','',true);
		$data['footer'] = $this->load->view('admin/elements/footer','',true);
		$data['maincontent']=$this->load->view('admin/maincontents/add-edit-student-message-view',$data,true);
		$this->load->view('admin/layout-after-login',$data);
	}
